...
if (sscanf(argv[1], "%u", &numOperations) != 1 || sscanf(argv[2], "%u",
&maxHeapSize) != 1) {
...
std::string opNum = std::string(totalZeros - opZeros,
'0').append(std::to_string(op));
nlohmann::json newOp;
// Add random integer to array
HeapOperation operation =
((op < initialInserts) ? HeapOperation::insert : (HeapOperation)
opDist(rng));
switch (operation) {
